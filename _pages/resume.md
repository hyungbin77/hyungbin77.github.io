...
* **'데이터 마이닝' 교과목 신규 개발 조교**, 빅데이터 혁신융합대학 사업단 — 2024.07 ~ 2024.12
  * 한 학기 분량 강의 콘텐츠와 PPT 전체 제작
  * 비지도학습 중심으로 데이터 시각화 · EDA, 유사 아이템 검색, 연관규칙분석, 차원축소, 군집분석, 이상치 탐색을 구성하고, 방법론마다 R · Python 실습 코드 개발
  * 현업 활용도가 높은 방법론 위주로 선별해 전처리부터 분석까지 실적용 가능한 형태로 설계
  * 교과목 업로드 전 강의 영상 최종 검토 — 오탈자 확인 및 대학 요구 양식에 맞춘 편집
